...
def rsmix(data_batch, label_batch, beta=1.0, n_sample=512, KNN=False, return_index=False):
    ...
    B, N, C = data_batch.shape
    pts_replaced = np.zeros((1, N, C))
    lam = np.zeros(B, dtype=float)
    lam_idx = np.zeros((B, N), dtype=bool)  # <--- New mask for visualization
    ...
    for i in range(data_batch.shape[0]):
        if pts_erase_idx[i][0][0]==data_batch.shape[1]:
            ...
            # Fill mask: first part from A (True), second from B (False)
            lam_idx[i, :len(tmp_pts_erased)] = True
            lam_idx[i, len(tmp_pts_erased):] = False
    ...
    if return_index:
        return data_batch_mixed, lam, label_a, label_b, lam_idx
    else:
